...
//查询文章额作者
func QueryArticle_Author(artID int)(string){
	fmt.Println(utils.QueryRowDB("select author from article where id=?"+strconv.Itoa(artID)))
	author := ""
	row := utils.QueryRowDB("select author from article where id=?"+strconv.Itoa(artID))
	row.Scan( &author)
	fmt.Println(author)
	art :=author
	return  art
